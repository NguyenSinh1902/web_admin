...
import serviceServices from "../services/api/serviceServices";

const SelectService = React.forwardRef(
    ({
        onSelected = () => { },
        isRequired = false,
        size = "large",
    ...
    // ref removed as it is unused
    {
        ...
        const OVG_spService_Select = React.useCallback(async () => {
            ...
            try {
                ...
                const res = await serviceServices.fetchAllServiceOptions();
                if (res?.length > 0) {
                    const op = res?.map((item) => ({
                        value: item.id,
                        label: item.optionName + " - " + item.optionPrice,
                        ...item,
                    }));
...
SelectService.displayName = "SelectGroupService";

export const SelectServiceOption = SelectService;
